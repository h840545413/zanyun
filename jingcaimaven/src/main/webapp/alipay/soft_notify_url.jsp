...
<%
	...
	if(AlipayNotify.verify(params)){//验证成功
		...
		BatchSql batch=new BatchSql();
		 sql = "if not EXISTS (SELECT * FROM t_alipaytb WHERE out_trade_no='"+out_trade_no+"') insert into t_alipaytb(body,buyer_email,buyer_id,exterface,is_success,notify_id,notify_time,"+
		...
		sql="if not EXISTS (SELECT * FROM t_money_details WHERE memo='"+out_trade_no+"') INSERT into t_money_details (operator_id,Createtime,money,zjx,status,memo) SELECT ton.operator_id,ton.createTime,ton.money,ton.zjx,ton.status,ton.tempTradeNo as memo FROM t_order_number ton WHERE ton.tempTradeNo='"+out_trade_no+"' ";
		 batch.addBatch(sql);
		//添加用户vip
		 sql="update t_user set vip=dateadd(YEAR,1,getDate())";
		 batch.addBatch(sql);
		 db.doInTransaction(batch);
				
			//	out.println(" TRADE_FINISHED out_trade_no:" + out_trade_no + "---trade_no:" + trade_no + "---trade_status:" + trade_status);
			//注意：
			//该种交易状态只在两种情况下出现
			//1、开通了普通即时到账，买家付款成功后。
			...
			//判断该笔订单是否在商户网站中已经做过处理
				//如果没有做过处理，根据订单号（out_trade_no）在商户网站的订单系统中查到该笔订单的详细，并执行商户的业务程序
				//如果有做过处理，不执行商户的业务程序
				out.println(" TRADE_SUCCESS out_trade_no:" + out_trade_no + "---trade_no:" + trade_no + "---trade_status:" + trade_status);
			//注意：
